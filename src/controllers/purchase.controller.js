...
exports.productList = async (req, res) => {
  var lp = await LP.findOne({ _id: req.params._id }).populate(
    'products.product'
  );
  ...
  LP.findOne({ _id: req.params.invc })
    ...
    .exec(async (err, doc) => {
      ...
      for (var i = 0; i < doc.products.length; i++) {
        var docs = await Serials.find({
          $and: [{ pid: doc.products[i].product._id }, { lp: req.params.invc }],
        });
...
exports.SaveLocalPurchase = async (req, res) => {
  ...
  if (lp === null) {
    ...
    (obj.products = [product]), (obj.subTotal = total);
