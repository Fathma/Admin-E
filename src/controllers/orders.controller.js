...
exports.OrdersByMonthPage = async (req, res) => {
  ...
  let orderlist = orders.filter((data) => {
    if (
      new Date(data.created).getMonth() + 1 === month &&
      new Date(data.created).getFullYear() === year
    ) {
      return data;
    }
  });
...
exports.saveSerialInOrders = async (req, res) => {
  ...
  serials.map(async (serial) => {
    ...
    var id = await Serial.findOne({
      $and: [
        { pid: req.params.model_id },
        { $or: [{ number: serial }, { sid: serial }] },
      ],
    });
...
exports.addSerialToProductPage = (req, res) => {
  get_orders({ _id: req.params.oid }, (order) => {
    Product.findOne({ _id: req.params.pid }, async function (err, docs) {
      let serial = await Serial.find({
        $and: [{ pid: req.params.pid }, { status: 'In Stock' }],
      });
        ...
        if (status === 'Delivered') {
          ...
          rs2.cart.map((item) => {
            item.serials.map(async (serial) => {
              await Serial.update(
                { _id: serial },
                { $set: { status: 'Delivered', invoice: invoice._id } }
              );
            });
          });
        }
