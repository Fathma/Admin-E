...
mongoose.connect(keys.database.mongoURI, (err) => {
  if (!err)
    logger.info('MongoDB connection Established, ' + keys.database.mongoURI);
  else
    logger.inf('Error in DB connection :' + JSON.stringify(err, undefined, 2));
});
...
app.listen(port, () =>
  logger.info('The server is live on http://127.0.0.1:3000/')
);
